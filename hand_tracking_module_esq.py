...
# Funções para gestos complementares - MODIFICADAS PARA MÃO ESQUERDA
def check_fingers_up(landmarks_list):
    ...
    # Pontas dos dedos e suas juntas de referência - MODIFICADO PARA MÃO ESQUERDA
    reference_points = [
        (4, 3, 2, 'x'),   # Polegar (usa coordenada x) - INVERTIDO PARA MÃO ESQUERDA
        (8, 6, 7, 'y'),   # Indicador
        (12, 10, 11, 'y'),  # Médio
        (16, 14, 15, 'y'),  # Anelar
        (20, 18, 19, 'y')   # Mínimo
    ...
    for i, (tip, joint1, joint2, axis) in enumerate(reference_points):
        if i == 0:  # Polegar - MODIFICADO PARA MÃO ESQUERDA
            # MÃO ESQUERDA: polegar para dentro (abaixado) quando está à DIREITA da junta
            if landmarks_list[tip][1] > landmarks_list[joint1][1]:
                fingers[0] = 1
        else:  # Outros dedos (mesma lógica)
